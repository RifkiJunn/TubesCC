<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-2xl font-bold text-gray-800 dark:text-white">Edit Barang</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400">{{ $product->title }}</p>
            </div>
        </div>
    </x-slot>
                    ...
                    <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div>
                            ...
                            @if($product->image)
                                <div class="mb-3 flex items-start gap-4">
                                    <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->title }}" class="h-24 w-24 object-cover rounded-xl border border-gray-200 dark:border-gray-700 shadow">
                                    <div class="text-sm text-gray-500 dark:text-gray-400">
                                        <p class="font-medium">Foto saat ini</p>
                                        <p class="text-xs">Biarkan kosong jika tidak ingin mengganti</p>
                                    </div>
                                </div>
                            @endif
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="4" required
                                class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition text-gray-800 dark:text-white resize-none">{{ old('description', $product->description) }}</textarea>
                        </div>
                        ...
                        <div class="bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20 p-5 rounded-2xl border border-indigo-100 dark:border-indigo-800">
                            <div class="flex items-center gap-3 mb-4">
                                ...
                                <div>
                                    <h4 class="font-semibold text-gray-800 dark:text-white">Lokasi GPS</h4>
                                    <p class="text-xs text-gray-500 dark:text-gray-400">Perbarui lokasi COD jika berubah</p>
                                </div>
                            </div>
                            ...
                            <div class="flex flex-col sm:flex-row gap-3">
                                ...
                                <span id="status-gps" class="flex items-center text-sm {{ $product->latitude ? 'text-green-600 font-medium' : 'text-gray-500 dark:text-gray-400' }}">
                                    @if($product->latitude)
                                        <svg class="w-4 h-4 mr-1.5 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                                        Koordinat tersimpan
                                    @else
        ...
        function getLocation() {
            ...
            navigator.geolocation.getCurrentPosition(
                (position) => {
                    ...
                    status.innerHTML = '<svg class="w-4 h-4 mr-1.5 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg> Lokasi diperbarui!';
